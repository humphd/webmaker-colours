...
console.log("Testing YouTube", url, /^(https|http)\:\/\/www\.youtube\.com\/embed.+/.test(url));
...
console.log("Testing Vimeo", url, /^(https|http)\:\/\/player\.vimeo\.com\/video.+/.test(url));
...
console.log("Testing SoundCloud", url, /^(https|http)\:\/\/w\.soundcloud\.com\/player.+/.test(url));
  return /^(https|http)\:\/\/w\.soundcloud\.com\/player.+/.test(url);
}

function filter(urls, embedFilterFns) {
  var filtered = [];
...
exports.filters = {
  video: function(urls) {
    return filter(urls, [YouTubeFilter, VimeoFilter]);
  },
  audio: function(urls) {
    return filter(urls, [SoundCloudFilter]);
  }
};
